...
import com.hb712.gleak_android.BuildConfig;
...
public abstract class OKHttpListener<T extends BaseBean> {
    ...
    public void onNetworkError(BaseBean baseBean) {//连接失败返回
        ...
        CommonUtils.toastInstance(st);
    }

    public void onServiceError(BaseBean baseBean) {//服务器返回非success,例如message="无效请求"
        CommonUtils.toastInstance(TextUtils.isEmpty(baseBean.message) ? "获取信息失败!" : BuildConfig.DEBUG ? (baseBean.message + baseBean.detailMessage) : baseBean.message);
    }
